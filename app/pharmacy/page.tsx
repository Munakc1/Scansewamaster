...
import { fetchPharmacyTransactions, fetchAllPharmacies, calculateSummary, PharmacyTransaction, PharmacySummary, PharmacyInfo } from '@/lib/api/pharmacy';
...
// Removed redundant interfaces (Pharmacy, PharmacyTransaction, PharmacySummary) since they’re imported from lib/api/pharmacy.ts
...
const PharmacyFinances = () => {
  ...
  const [pharmacies, setPharmacies] = useState<PharmacyInfo[]>([]); // Added to store pharmacy details
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Replaced mock data fetch with fetchPharmacyTransactions
        const transactionData = await fetchPharmacyTransactions();
        setTransactions(transactionData);

        // Fetch pharmacy details for modal enhancement
        const pharmacyData = await fetchAllPharmacies();
  ...
  const handleExport = (type: 'csv' | 'json') => {
    const dataToExport = visibleTransactions.map((t) => ({
      ...
      'Pharmacy': t.pharmacyName, // Changed to pharmacyName
  ...
  const visibleTransactions = transactions.filter((t) => {
    const matchesSearch =
      search === '' ||
      t.pharmacyName.toLowerCase().includes(search.toLowerCase()) || // Changed to pharmacyName
      t.transactionId.toLowerCase().includes(search.toLowerCase()) ||
      (t.orderId && t.orderId.toLowerCase().includes(search.toLowerCase()));

    const matchesStatus = statusFilter === 'all' || t.status === statusFilter;

    const matchesPharmacy = pharmacyFilter === 'all' || t.pharmacyId === pharmacyFilter;
  ...
  const pharmacySummaries = calculateSummary(visibleTransactions); // Using imported calculateSummary
  const allPharmacies = Array.from(
    new Set(
      transactions.map((t) => ({
        id: t.pharmacyId,
        name: t.pharmacyName, // Changed to pharmacyName
      }))
    )
  );
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
          <div>
            ...
            <p className={`mt-1 ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
              View and manage financial transactions with partner pharmacies
            </p>
          </div>
          ...
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <XAxis dataKey="name" stroke={darkMode ? '#D1D5DB' : '#4B5563'} tick={{ fill: darkMode ? '#F3F4F6' : '#1F2937' }} />
                <YAxis yAxisId="left" orientation="left" stroke={darkMode ? '#93C5FD' : '#3B82F6'} tick={{ fill: darkMode ? '#F3F4F6' : '#1F2937' }} />
                <YAxis yAxisId="right" orientation="right" stroke={darkMode ? '#6EE7B7' : '#10B981'} tick={{ fill: darkMode ? '#F3F4F6' : '#1F2937' }} />
                <Tooltip
                ...
                />
                <Legend wrapperStyle={{ color: darkMode ? '#F3F4F6' : '#1F2937', paddingTop: '20px' }} />
                <Bar yAxisId="left" dataKey="revenue" name="Revenue ($)" fill={darkMode ? '#3B82F6' : '#8884d8'} radius={[4, 4, 0, 0]} />
                <Line
                  yAxisId="right"
                  type="monotone"
                  dataKey="orders"
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <div
                className={`p-3 rounded-full ${darkMode ? 'bg-blue-900 text-blue-400' : 'bg-blue-100 text-blue-600'}`}
              >
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <div
                className={`p-3 rounded-full ${darkMode ? 'bg-green-900 text-green-400' : 'bg-green-100 text-green-600'}`}
              >
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <div
                className={`p-3 rounded-full ${darkMode ? 'bg-purple-900 text-purple-400' : 'bg-purple-100 text-purple-600'}`}
              >
                  ...
                  <tr key={pharmacy.pharmacyId} className={darkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className={`text-sm font-medium ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                        {pharmacy.pharmacyName} {/* Changed to pharmacyName */}
                      </div>
                ...
                <thead className={darkMode ? 'bg-gray-700' : 'bg-gray-50'}>
                  <tr>
                    ...
                    <th
                      className={`px-6 py-3 text-left text-xs font-medium uppercase tracking-wider ${
                        darkMode ? 'text-gray-300' : 'text-gray-500'
                      }`} // Fixed typo by adding opening backtick
                    >
                      Commission
                    </th>
                    <th
                ...
                <tbody className={`divide-y divide-gray-200 ${darkMode ? 'bg-gray-800 divide-gray-700' : 'bg-white'}`}>
                  {visibleTransactions.map((transaction) => (
                    <tr key={transaction.id} className={darkMode ? 'hover:bg-gray-700' : 'hover:bg-gray-50'}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className={`text-sm font-medium ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                          {transaction.pharmacyName} {/* Changed to pharmacyName */}
                        </div>
                        <div className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                          {transaction.pharmacyId}
                        </div>
                      </td>
                      <td className={`px-6 py-4 whitespace-nowrap text-sm ${darkMode ? 'text-gray-300' : 'text-gray-900'}`}>
          ...
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                <CartesianGrid strokeDasharray="3 3" stroke={darkMode ? '#4B5563' : '#E5E7EB'} />
                <XAxis dataKey="name" stroke={darkMode ? '#D1D5DB' : '#4B5563'} tick={{ fill: darkMode ? '#F3F4F6' : '#1F2937' }} />
                <YAxis stroke={darkMode ? '#D1D5DB' : '#4B5563'} tick={{ fill: darkMode ? '#F3F4F6' : '#1F2937' }} />
                <Tooltip
                ...
                />
                <Legend wrapperStyle={{ color: darkMode ? '#F3F4F6' : '#1F2937', paddingTop: '20px' }} />
                <Bar dataKey="transactions" fill={darkMode ? '#8B5CF6' : '#8884d8'} name="Total Transactions" radius={[4, 4, 0, 0]} />
                <Bar dataKey="revenue" fill={darkMode ? '#10B981' : '#82ca9d'} name="Total Revenue (₹)" radius={[4, 4, 0, 0]} />
                <Bar
                  dataKey="commission"
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between">
                      ...
                      <span className={`text-sm font-medium ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                        {selectedTransaction.pharmacyName} {/* Changed to pharmacyName */}
                      </span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between">
                      <span className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-500'}`}>City:</span>
                      <span className={`text-sm font-medium ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                        {pharmacies.find((p) => p.pharmacyId === selectedTransaction.pharmacyId)?.location.city || 'N/A'} {/* Added city from pharmacies */}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className={`text-sm ${darkMode ? 'text-gray-300' : 'text-gray-500'}`}>Phone:</span>
                      <span className={`text-sm font-medium ${darkMode ? 'text-white' : 'text-gray-900'}`}>
                        {pharmacies.find((p) => p.pharmacyId === selectedTransaction.pharmacyId)?.contact.phone || 'N/A'} {/* Added phone from pharmacies */}
                      </span>
                    </div>
